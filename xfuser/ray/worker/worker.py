...
class DiTWorker(WorkerBase):
    ...
    def init_worker_distributed_environment(self):
        print("init_worker_distributed_environment111", self.rank, self.parallel_config.world_size)
        init_distributed_environment(
            rank=self.rank,
...
class VAEWorker(WorkerBase):
    ...
    def init_worker_distributed_environment(self):
        print(f"init_worker_distributed_environment222 {self.rank},{self.parallel_config.vae_parallel_size}")
        init_distributed_environment(
            rank=self.rank,
            world_size=self.parallel_config.world_size,
        )
